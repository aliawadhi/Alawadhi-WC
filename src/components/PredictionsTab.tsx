...
export default function PredictionsTab({
  ...
  const [lootChoices, setLootChoices] = useState<
    Record<string, "flat_3" | "double_down" | "insurance">
  >({});
  ...
  const [expandedOtherPreds, setExpandedOtherPreds] = useState<
    Record<string, boolean>
  >({});
  const [otherPredictionsMap, setOtherPredictionsMap] = useState<
    Record<string, any[]>
  >({});
  const [loadingOtherPreds, setLoadingOtherPreds] = useState<
    Record<string, boolean>
  >({});
  const [selectedLeagueIdForMatch, setSelectedLeagueIdForMatch] = useState<
    Record<string, string>
  >({});

  const fetchOtherPredictions = async (
    matchId: string,
    leagueIdToFetch: string,
  ) => {
    ...
    try {
      ...
      const mMatchObj = (matches || []).find(
        (item) => item.match_id === matchId,
      );
      const mKickoff = mMatchObj
        ? new Date(mMatchObj.kickoff_time).getTime()
        : 0;
      const nowTime = Date.now();
      const isPermanentlyLocked = mMatchObj
        ? mKickoff - nowTime < 3600000
        : false;

      const homeR = mMatchObj
        ? (mMatchObj.home_rank ?? TEAM_RANKS[mMatchObj.home_team] ?? 60)
        : 60;
      const awayR = mMatchObj
        ? (mMatchObj.away_rank ?? TEAM_RANKS[mMatchObj.away_team] ?? 60)
        : 60;
      const isGiantSlayer = mMatchObj
        ? mMatchObj.is_giant_slayer === true ||
      ...
      const mappedList = members.map((mMember) => {
        ...
        const userPredObj = (preds || []).find(
          (p) => p.user_id === mMember.user_id,
        );

        if (!userPredObj) {
          const defaultedPts =
            mMatchObj &&
            mMatchObj.home_score_final !== null &&
            mMatchObj.away_score_final !== null
              ? calculatePoints(
                  0,
                  0,
        ...
        const isChestUnopened =
          isLoot &&
          isExact &&
          (userPredObj.points_earned === null ||
            userPredObj.points_earned === undefined);
  ...
  const toggleOtherPreds = (matchId: string) => {
    ...
    if (isExpanding) {
      const targetLeagueId =
        selectedLeagueIdForMatch[matchId] ||
        activeLeagueId ||
        (joinedLeagues && joinedLeagues[0]?.league_id);
      if (targetLeagueId) {
        if (!selectedLeagueIdForMatch[matchId]) {
          setSelectedLeagueIdForMatch((prev) => ({
            ...prev,
            [matchId]: targetLeagueId,
          }));
        }
  ...
  const triggerDialog = (
    ...
    msgAr: string,
    msgEn: string,
    type: "double_down" | "underdog_specialist" | "error" | "info" = "error",
  ) => {
    setErrorDialog({
      isOpen: true,
      titleAr,
  ...
  const withScrollStabilization = async (
    matchId: string,
    buttonId: string,
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (triggerReload) {
        console.log(
          "Forcing page refresh because an active match kickoff has passed the 1-hour-pre-kickoff lock threshold.",
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const visibleUntil =
        nowMs < round1UnlockTime
          ? round1End
          : nowMs < round2UnlockTime
            ? round2End
            : round3End;
      ...
      if (matchesData) {
        ...
        setMatches((prev) => {
          if (
            prev &&
            prev.length > 0 &&
            prev.length === uniqueFiltered.length
          ) {
            const prevKeys = prev
              .map(
                (p) =>
                  `${p.match_id}_${p.home_score_final}_${p.away_score_final}`,
              )
              .join(",");
            const newKeys = uniqueFiltered
              .map(
                (p) =>
                  `${p.match_id}_${p.home_score_final}_${p.away_score_final}`,
              )
        ...
        if (
          !initialChangeableMatchIdsRef.current &&
      ...
      if (user) {
        ...
        try {
          ...
          const row0 = tokenRows?.find(
            (r) => r.match_id === "00000000-0000-0000-0000-000000000000",
          );
          const row1 = tokenRows?.find(
            (r) => r.match_id === "00000000-0000-0000-0000-000000000001",
          );
          ...
          const isAlgeriaAustriaFinalized = matchesData?.some(
            (m) =>
          ...
          if (!isAlgeriaAustriaFinalized) {
            ...
            supabase
              ...
              .then(({ error }) => {
                if (error)
                  console.error(
                    "Error cleaning up comeback award prediction:",
                    error,
                  );
              });
          } else if (row1) {
            ...
            const localCbDoubleVal = localStorage.getItem(
              `CB_double_tokens_${user.id}`,
            );
            if (localCbDoubleVal !== null) {
              cbDoubleTokens = parseInt(localCbDoubleVal);
            }
            const localCbTripleVal = localStorage.getItem(
              `CB_triple_tokens_${user.id}`,
            );
        ...
        if (predictionsData) {
          ...
          const matchMap = new Map<string, any>(
            (matchesData || []).map((m) => [m.match_id, m]),
          );
          ...
          predictionsData.forEach((pred) => {
            ...
            if (m) {
              ...
              const isFinished =
                m.home_score_final !== null &&
                m.home_score_final !== undefined &&
                m.away_score_final !== null &&
                m.away_score_final !== undefined &&
                !isLive;
              if (isFinished) {
                const hasHome =
                  pred.predicted_home_score !== null &&
                ...
                if (hasHome && hasAway) {
                  ...
                  const isChestClaimed =
                    pred.points_earned !== null &&
                    pred.points_earned !== undefined;

                  if (isLoot && isExact && isChestClaimed) {
                    const factor = getDeterministicUserMatchFactor(
                      user.id,
                      m.match_id,
                      m.group_stage,
                    );
          ...
          localStorage.setItem(
            `CB_double_tokens_${user.id}`,
            cbDoubleTokens.toString(),
          );
          localStorage.setItem(
            `CB_triple_tokens_${user.id}`,
            cbTripleTokens.toString(),
          );
          ...
          predictionsData.forEach((pred) => {
            if (
              pred.match_id === "00000000-0000-0000-0000-000000000000" ||
              pred.match_id === "00000000-0000-0000-0000-000000000001"
            )
              return;
            ...
            if (!isFinished) {
              ...
              const storedChestKey = localStorage.getItem(
                `open_chest_${pred.match_id}`,
              );
              if (
                !isChestClaimed &&
                storedChestKey &&
                storedChestKey !== activeSalt
              ) {
                localStorage.removeItem(`open_chest_${pred.match_id}`);
                localStorage.removeItem(`loot_result_${pred.match_id}`);
              }
              openMap[pred.match_id] =
                isChestClaimed ||
                localStorage.getItem(`open_chest_${pred.match_id}`) !== null;
            }
          });
          ...
          setPredictions((prev) => {
            const keys1 = Object.keys(prev || {})
              ...
              .map(
                (k) =>
                  `${k}_${prev[k].home}_${prev[k].away}_${prev[k].is_joker}_${prev[k].is_insurance}`,
              )
              .join(",");
            const keys2 = Object.keys(predsMap)
              .sort()
              .map(
                (k) =>
                  `${k}_${predsMap[k].home}_${predsMap[k].away}_${predsMap[k].is_joker}_${predsMap[k].is_insurance}`,
              )
              .join(",");
            if (keys1 === keys2) {
              return prev;
          ...
          setSaved((prev) => {
            const keys1 = Object.keys(prev || {})
              ...
              .map((k) => `${k}_${prev[k]}`)
              .join(",");
            const keys2 = Object.keys(savedMap)
              .sort()
              .map((k) => `${k}_${savedMap[k]}`)
              .join(",");
            if (keys1 === keys2) {
              return prev;
            }
          ...
          setLootChoices((prev) => {
            const keys1 = Object.keys(prev || {})
              ...
              .map((k) => `${k}_${prev[k]}`)
              .join(",");
            const keys2 = Object.keys(lootMap)
              .sort()
              .map((k) => `${k}_${lootMap[k]}`)
              .join(",");
            if (keys1 === keys2) {
          ...
          setOpenedChests((prev) => {
            const keys1 = Object.keys(prev || {})
              ...
              .map((k) => `${k}_${prev[k]}`)
              .join(",");
            const keys2 = Object.keys(openMap)
              .sort()
              .map((k) => `${k}_${openMap[k]}`)
              .join(",");
            if (keys1 === keys2) {
          ...
          if (allMatchesData) {
            ...
            allMatchesData.forEach((match) => {
              if (match.match_id === "00000000-0000-0000-0000-000000000000")
                return;
              if (match.group_stage?.includes("[HIDDEN]")) return;
              const isLive = match.group_stage?.includes("[LIVE]");
              const isFinished =
                match.home_score_final !== null &&
                match.home_score_final !== undefined &&
                match.away_score_final !== null &&
                match.away_score_final !== undefined;
              ...
              const p =
                rawP &&
                rawP.predicted_home_score !== null &&
                rawP.predicted_home_score !== undefined &&
                rawP.predicted_away_score !== null &&
                rawP.predicted_away_score !== undefined
                  ? rawP
                  : {
              ...
              const chestOpened =
                !earnedLootChest ||
                localStorage.getItem(`open_chest_${match.match_id}`) === "true";
              const hasDbPoints =
                p.points_earned !== null && p.points_earned !== undefined;
              if (!chestOpened && !hasDbPoints) return;

              const homeRank =
                match.home_rank ?? TEAM_RANKS[match.home_team] ?? 60;
              const awayRank =
                match.away_rank ?? TEAM_RANKS[match.away_team] ?? 60;
              const isGS =
                match.is_giant_slayer === true ||
                (Math.abs(homeRank - awayRank) >= 35 &&
                  (homeRank <= 20 || awayRank <= 20));
            ...
            setUserStats((prev) => {
              if (
                ...
                prev.slayerPoints === slayerPoints &&
                prev.exactCount === exactCount
              ) {
                return prev;
              }
    ...
    <div className="predictions-wrap">
      ...
      >
        <h2
          ...
          style={{ fontFamily: isAr ? "Cairo, system-ui" : undefined }}
        >
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          >
            <span>🏆</span>{" "}
            {isAr ? "دليل نقاط خروج المغلوب" : "Knockout Scoring Guide"}
          </button>
        </div>
      </div>
      ...
      {showRules && (
        <div
          className="rules-panel-container"
          style={{
        ...
        >
          ...
          >
            <div className="rules-box rules-box-standard">
              ...
              >
                {t("rulesStandardTitle")}
              </h4>
              <ul
                className="rules-list"
                style={{
                  fontSize: "0.8rem",
                  paddingLeft: isAr ? 0 : "1.1rem",
                  paddingRight: isAr ? "1.1rem" : 0,
                  display: "flex",
                  flexDirection: "column",
                  gap: "0.25rem",
                }}
              >
                <li>
                  <strong className="rules-highlight-blue">
                    {isAr ? "٥ نقاط" : "5 Points"}
                  </strong>
                  : {t("rulesStandard1")}
                </li>
                <li>
                  <strong className="rules-highlight-blue">
                    {isAr ? "نقطتان" : "2 Points"}
                  </strong>
                  : {t("rulesStandard2")}
                </li>
                <li>
                  <strong className="rules-highlight-red">
                    {isAr ? "٠ نقاط" : "0 Points"}
                  </strong>
                  : {t("rulesStandard3")}
                </li>
                <li>
                  <strong className="rules-highlight-green">
                    {t("rulesStandard4")}{" "}
                  </strong>
                </li>
              </ul>
            ...
            <div className="rules-box rules-box-slayer">
              ...
              >
                ...
                <li
                  className="rules-warning-item"
                  style={{
                    listStyleType: "none",
                    marginLeft: isAr ? 0 : "-1.1rem",
                    marginRight: isAr ? "-1.1rem" : 0,
                  }}
                >
                  {t("rulesSlayerWarn")}
                </li>
            ...
            <div className="rules-box rules-box-underdog">
              ...
              >
                {t("rulesUnderdogHeader")}
              </h4>
              <p
                className="rules-desc"
                style={{ fontSize: "0.78rem", marginBottom: "0.25rem" }}
              >
                {t("rulesUnderdogDesc")}
              </p>
            </div>
            ...
            <div className="rules-box rules-box-loot">
              ...
              >
                <li>
                  <strong>
                    {isAr ? "١. ابحث عن الشعار:" : "1. Spot the Badge:"}
                  </strong>{" "}
                  {isAr
                    ? "ابحث عن لقاء يحمل شعار صندوق الغنائم 🎁 والتوهج الذهبي."
                    : "Look for matches marked with the 🎁 SURPRISE LOOT badge & golden glow."}
                </li>
                <li>
                  <strong>
                    {isAr ? "٢. توقع النتيجة بدقة:" : "2. Get the Exact Score:"}
                  </strong>{" "}
                  {isAr
                    ? "سجّل توقعك لنتيجة المباراة وأمّنه قبل بدء اللقاء. يجب أن تُصيب النتيجة الصحيحة تماماً للتأهل لكسب الصندوق!"
                    : "Submit & lock in your score prediction. You must get the score perfectly right to earn the chest!"}
                </li>
              </ul>
            </div>

            <div
              className="rules-box rules-box-knockout"
              style={{
                border: "1px solid #10b981",
                borderRadius: "6px",
                padding: "0.75rem",
            ...
            >
              ...
              >
                🏆{" "}
                {isAr
                  ? "مضاعف دور خروج المغلوب"
                  : "Knockout Stage Double Points"}
              </h4>
              <p
                className="rules-desc"
                style={{
              ...
              >
                <li>
                  <strong style={{ color: "#34d399" }}>
                    {isAr ? "١٠ نقاط" : "10 Points"}
                  </strong>
                  :{" "}
                  {isAr
                    ? "للتوقع الصحيح للنتيجة الدقيقة (بدلاً من ٥ نقاط)."
                    : "For guessing the exact score (instead of 5 pts)."}
                </li>
                <li>
                  <strong style={{ color: "#34d399" }}>
                    {isAr ? "٤ نقاط" : "4 Points"}
                  </strong>
                  :{" "}
                  {isAr
                    ? "لتوقع فائز صحيح أو تعادل ولكن بنتيجة خاطئة (بدلاً من نقطتين)."
                    : "For guessing correct outcome but incorrect score (instead of 2 pts)."}
                </li>
                <li
                  style={{
                    listStyleType: "none",
                ...
                >
                  💡{" "}
                  {isAr
                    ? "ملاحظة: تُحسب مضاعفات قاهر العمالقة والـ Double Down بشكل تراكمي فوق هذه النقاط الإقصائية!"
                    : "Note: Giant Slayer and Double Down multipliers apply cumulatively on top of these doubled knockout points!"}
                </li>
              </ul>
            </div>

            <div
              className="rules-box rules-box-tokens"
              style={{
                border: "1px solid var(--gold)",
                borderRadius: "6px",
                padding: "0.75rem",
            ...
            >
              ...
              >
                🤠{" "}
                {isAr
                  ? "طاقات اللعب والبطاقات الفعّالة"
                  : "Power-Up Tokens Guide"}
              </h4>
              <p
                className="rules-desc"
                style={{
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <span>
                      {isAr
                        ? "خبير المستضعفين (المتوفر لديك: " +
                          insuranceTokens +
                          ")"
                        : "Underdog Specialist (Available: " +
                          insuranceTokens +
                          ")"}
                    </span>
                  </div>
                ...
                >
                  ...
                  >
                    ...
                    <span>
                      {isAr
                        ? "مضاعفة النقاط (المتوفر لديك: " +
                          doubleDownTokens +
                          ")"
                        : "Double Down (Available: " + doubleDownTokens + ")"}
                    </span>
                  </div>
                  <span style={{ opacity: 0.9, lineHeight: "1.4" }}>
                    {isAr
            ...
            <div className="predictions-list">
              ...
              {sortedMatches.map((m, index) => {
                ...
                const mIsFinished =
                  ...
                  m.away_score_final !== null &&
                  !mIsLive;
                const isFirstFinishedMatch =
                  mIsFinished &&
                  (index === 0 ||
                    !(() => {
                      const prev = sortedMatches[index - 1];
                      const prevIsFinished =
                        prev &&
                        prev.home_score_final !== null &&
                        prev.away_score_final !== null &&
                        !prev.group_stage?.includes("[LIVE]");
                      return prevIsFinished;
                    })());
                const shouldHideFinishedCard = mIsFinished && isRound1Collapsed;
                const renderActiveUpcomingHeader =
                  index === 0 && !mIsFinished && finishedMatches.length > 0;
                          ...
                          >
                            ...
                            <div style={{ textAlign: isAr ? "right" : "left" }}>
                              ...
                              <p className="header-subtitle">
                                {isAr
                                  ? `${finishedMatches.length} مباراة - انقر للتوسيع وعرض التوقعات الماضية`
                                  : `${finishedMatches.length} matches - Click to expand and view past predictions`}
                              </p>
                            </div>
                          </div>
                          <div
                ...
                const hasSurpriseLoot = isSurpriseLoot(
                  m.home_team,
                  m.away_team,
                  m.match_id,
                  userId,
                ...
                const showIns =
                  !hasSurpriseLoot && (isHomeUnderdog || isAwayUnderdog);
                ...
                return (
                  <React.Fragment key={m.match_id}>
                      ...
                      >
                        ...
                        >
                          ...
                          <div style={{ textAlign: isAr ? "right" : "left" }}>
                            ...
                            <p className="header-subtitle">
                              {isAr
                                ? `${finishedMatches.length} مباراة - انقر للإخفاء`
                                : `${finishedMatches.length} matches - Click to collapse`}
                            </p>
                          </div>
                        </div>
                        <div
                    ...
                    >
                      ...
                      <div
                        className="prediction-card-header"
                        style={{
                          display: "flex",
                          justifyContent: "space-between",
                          alignItems: "center",
                      ...
                      >
                        <span className="flex flex-col sm:flex-row sm:items-baseline gap-y-0.5 sm:gap-x-1.5">
                          <span>
                            🏟️{" "}
                            {m.kickoff_time
                              ? getFormattedDate(m.kickoff_time)
                              : isAr
                                ? "لم يحدد الوقت"
                                : "Time TBD"}
                          </span>
                        ...
                        {isLive ? (
                          <span
                            style={{
                              ...
                              gap: "0.25rem",
                              fontWeight: "bold",
                              marginLeft: isAr ? "unset" : "auto",
                              marginRight: isAr ? "auto" : "unset",
                          ...
                          <span
                            style={{
                              color: isDefaulted00 ? "#94a3b8" : "#EF4444",
                              display: "flex",
                              alignItems: "center",
                              gap: "0.25rem",
                              fontWeight: "bold",
                          ...
                          <span
                            className="pred-status-locked-in"
                            style={{
                              display: "flex",
                              alignItems: "center",
                              gap: "0.25rem",
                          ...
                          >
                            {isAr
                              ? "تم تأكيد التوقع (يُغلق باب التعديلات قبل ساعة واحدة من انطلاق المباراة)"
                              : "✓ Prediction Locked In (Edits close 1h before kickoff)"}
                          </span>
                        ) : (
                          <span
                            className="pred-status-open"
                            style={{
                              marginLeft: isAr ? "unset" : "auto",
                              marginRight: isAr ? "auto" : "unset",
                              textAlign: isAr ? "left" : "right",
                      ...
                      >
                        {isGiantSlayer && (
                          <div
                            className="giant-badge"
                            style={{
                              top: "-11px",
                              right: isAr ? "unset" : "15px",
                              left: isAr ? "15px" : "unset",
                            }}
                          >
                            <span className="giant-badge-icon">⚡</span>
                            <span>
                              {isAr ? "قاهر العمالقة" : "GIANT SLAYER"}
                            </span>
                          </div>
                        )}
                        ...
                        {hasSurpriseLoot && (
                          ...
                          >
                            ...
                            <span>
                              {isAr ? "غنائم مفاجئة" : "SURPRISE LOOT"}
                            </span>
                            <span className="loot-badge-emoji">🎁</span>
                          </div>
                        )}
                                ...
                                {homeR != null && (
                                  <span
                                    style={{
                                      ...
                                      marginLeft: isAr ? 0 : "0.25rem",
                                      marginRight: isAr ? "0.25rem" : 0,
                                      fontWeight: "normal",
                                    }}
                                  >
                                    ({homeR})
                          ...
                          >
                            ...
                            {showDD && (
                              <button
                                ...
                                type="button"
                                onClick={async () => {
                                  await withScrollStabilization(
                                    m.match_id,
                                    `btn-double-down-${m.match_id}`,
                                    async () => {
                                      const currentMatch =
                                        matches.find(
                                          (matchObj) =>
                                            matchObj.match_id === m.match_id,
                                        ) || m;
                                      const isFinalized =
                                        currentMatch.home_score_final !==
                                          null &&
                                        currentMatch.away_score_final !== null;
                                      const kickoffTime = new Date(
                                        currentMatch.kickoff_time,
                                      ).getTime();
                                      const now = Date.now();
                                      const canToggle =
                                        kickoffTime - now >= 3600000 &&
                                        !isFinalized;
                                      ...
                                      if (isCurrentlyJoker) {
                                        // Refund token
                                        let homeVal =
                                          predictions[m.match_id]?.home ?? "";
                                        let awayVal =
                                          predictions[m.match_id]?.away ?? "";
                                        ...
                                        if (doubleDownTokens > 0) {
                                          ...
                                          const isCurrentlyCbDouble =
                                            predictions[m.match_id]
                                              ?.is_comeback_double ?? false;
                                          let refundCbDoubleCount =
                                            comebackDoubleTokens;
                                          if (isCurrentlyCbDouble) {
                                            refundCbDoubleCount += 1;
                                          }

                                          // If Comeback Triple is active, refund it!
                                          const isCurrentlyCbTriple =
                                            predictions[m.match_id]
                                              ?.is_comeback_triple ?? false;
                                          let refundCbTripleCount =
                                            comebackTripleTokens;
                                          ...
                                          if (isCurrentlyInsured) {
                                            ...
                                            localStorage.setItem(
                                              `INS_tokens_${user.id}`,
                                              refundInsCount.toString(),
                                            );
                                          }
                                          if (isCurrentlyCbDouble) {
                                            setComebackDoubleTokens(
                                              refundCbDoubleCount,
                                            );
                                            localStorage.setItem(
                                              `CB_double_tokens_${user.id}`,
                                              refundCbDoubleCount.toString(),
                                            );
                                          }
                                          if (isCurrentlyCbTriple) {
                                            setComebackTripleTokens(
                                              refundCbTripleCount,
                                            );
                                            localStorage.setItem(
                                              `CB_triple_tokens_${user.id}`,
                                              refundCbTripleCount.toString(),
                                            );
                                          }
                                          setPredictions((prev) => ({
                                            ...prev,
                                ...
                                style={{
                                  backgroundColor: predictions[m.match_id]
                                    ?.is_joker
                                    ? "rgba(168, 85, 247, 0.12)"
                                    : "rgba(255, 255, 255, 0.02)",
                                  border: predictions[m.match_id]?.is_joker
                                    ? "1px solid #c084fc"
                                    : "1px dashed rgba(255, 255, 255, 0.15)",
                                  color: predictions[m.match_id]?.is_joker
                                    ? "#c084fc"
                                    : "var(--grey)",
                                  height: "36px",
                                  padding: "0 1rem",
                              ...
                              <button
                                  ...
                                  await withScrollStabilization(
                                    ...
                                    async () => {
                                      ...
                                      if (!isCurrentlyInsured) {
                                        ...
                                        const homeR =
                                          m.home_rank ??
                                          TEAM_RANKS[m.home_team] ??
                                          60;
                                        const awayR =
                                          m.away_rank ??
                                          TEAM_RANKS[m.away_team] ??
                                          60;

                                        const isHomeUnderdog = homeR > awayR;
                                        const isAwayUnderdog = awayR > homeR;

                                        const hasPredictedUnderdogToWin =
                                          hScoreNum !== null &&
                                          aScoreNum !== null &&
                                          ((isHomeUnderdog &&
                                            hScoreNum > aScoreNum) ||
                                ...
                                style={{
                                  backgroundColor: predictions[m.match_id]
                                    ...
                                    : "rgba(255, 255, 255, 0.02)",
                                  border: predictions[m.match_id]?.is_insurance
                                    ? "1px solid #38bdf8"
                                    : "1px dashed rgba(255, 255, 255, 0.15)",
                                  color: predictions[m.match_id]?.is_insurance
                                    ? "#38bdf8"
                                    : "var(--grey)",
                                  height: "36px",
                                  padding: "0 1rem",
                            ...
                            {isKnockout && (
                              <button
                                ...
                                onClick={async () => {
                                  await withScrollStabilization(
                                    m.match_id,
                                    `btn-comeback-double-${m.match_id}`,
                                    async () => {
                                      const currentMatch =
                                        matches.find(
                                          (matchObj) =>
                                      ...
                                      if (isCurrentlyCbDouble) {
                                        ...
                                        let awayVal =
                                          predictions[m.match_id]?.away ?? "";
                                        const newCbDouble =
                                          comebackDoubleTokens + 1;

                                        // Optimistic UI updates
                                        setComebackDoubleTokens(newCbDouble);
                                        localStorage.setItem(
                                          `CB_double_tokens_${user.id}`,
                                          newCbDouble.toString(),
                                        );
                                        setPredictions((prev) => ({
                                          ...prev,
                                          [m.match_id]: {
                                ...
                                style={{
                                  backgroundColor: predictions[m.match_id]
                                    ...
                                    : "rgba(255, 255, 255, 0.02)",
                                  border: predictions[m.match_id]
                                    ?.is_comeback_double
                                    ? "1px solid #3b82f6"
                                    : "1px dashed rgba(255, 255, 255, 0.15)",
                                  color: predictions[m.match_id]
                                    ?.is_comeback_double
                                    ? "#60a5fa"
                                    : "var(--grey)",
                                  height: "36px",
                              ...
                              >
                                {predictions[m.match_id]?.is_comeback_double ? (
                                  <>
                                    ...
                                    <strong>
                                      {isAr
                                        ? "مضاعف الريمونتادا x2 نشط"
                                        : "Remontada Double Active (x2)"}
                                    </strong>
                                    <span
                                      style={{
                                        fontSize: "0.65rem",
                            ...
                            {isKnockout && (
                              <button
                                ...
                                onClick={async () => {
                                  await withScrollStabilization(
                                    m.match_id,
                                    `btn-comeback-triple-${m.match_id}`,
                                    async () => {
                                      const currentMatch =
                                        matches.find(
                                          (matchObj) =>
                                      ...
                                      if (isCurrentlyCbTriple) {
                                        ...
                                        let awayVal =
                                          predictions[m.match_id]?.away ?? "";
                                        const newCbTriple =
                                          comebackTripleTokens + 1;

                                        // Optimistic UI updates
                                        setComebackTripleTokens(newCbTriple);
                                        localStorage.setItem(
                                          `CB_triple_tokens_${user.id}`,
                                          newCbTriple.toString(),
                                        );
                                        setPredictions((prev) => ({
                                          ...prev,
                                          [m.match_id]: {
                              ...
                              >
                                {predictions[m.match_id]?.is_comeback_triple ? (
                                  <>
                                    ...
                                    <strong>
                                      {isAr
                                        ? "مضاعف الريمونتادا x3 نشط"
                                        : "Remontada Triple Active (x3)"}
                                    </strong>
                                    <span
                                      style={{
                                        fontSize: "0.65rem",
                        ...
                        predictions[m.match_id]?.is_joker && (
                          ...
                          >
                            ...
                            >
                              ...
                              <span>
                                {isAr
                                  ? "تم تأمين مضاعف النقاط x2 للمباراة!"
                                  : "Double Down Multiplier Locked In (x2)!"}
                              </span>
                            </div>
                          </div>
                        )}
                      ...
                      {hasSurpriseLoot && !hasActualScore && (
                        <div
                          className="loot-selection-container"
                          style={{
                            marginTop: "0.4rem",
                            padding: "1rem",
                        ...
                        >
                          {isSaved ? (
                            ...
                            >
                              ...
                              >
                                {isAr
                                  ? "تم تأمين صندوق الغنائم الغامض! 🎁"
                                  : "Surprise Mystery Chest Secured! 🎁"}
                              </span>
                              <span
                                style={{
                                  fontSize: "0.67rem",
                            ...
                            >
                              ...
                              >
                                🎰{" "}
                                {isAr
                                  ? "جاري سحب جائزتك العشوائية..."
                                  : "ROLLING YOUR SURPRISE LOOT..."}
                              </span>
                            </div>
                          ) : (
                            <div
                            ...
                            >
                              ...
                              >
                                {isAr
                                  ? "صندوق هدايا عشوائي نشط لهذه المباراة!"
                                  : "Surprise Loot Active!"}
                              </span>
                              <p
                                style={{
                                  fontSize: "0.67rem",
                              ...
                              >
                                {isAr
                                  ? "قم بتأمين توقعك الدقيق. إذا أصبت النتيجة الصحيحة تماماً للمباراة، فستربح الصندوق وتكشف جائزته العشوائية عند صافرة النهاية!"
                                  : "Lock in your prediction. If your exact score guess is correct, you'll earn this chest to unlock your surprise reward after the final whistle!"}
                              </p>
                            </div>
                          )}
                        </div>
                          ...
                          <span>
                            ...
                            {pred.home === "" || pred.away === "" ? (
                              <span className="eligibility-text-none">
                                {isAr
                                  ? "أدخل توقعًا لتحديد أهلية مضاعف النقاط x2."
                                  : "Enter a prediction to evaluate x2 multiplier eligibility."}
                              </span>
                            ) : (isHomeUnderdog &&
                                Number(pred.home) >= Number(pred.away)) ||
                              (isAwayUnderdog &&
                                Number(pred.away) >= Number(pred.home)) ? (
                              <span
                                className="eligibility-text-eligible"
                                style={{ fontWeight: "bold" }}
                              >
                                {isAr
                                  ? "مؤهل لربح طاقة نقاط مضاعفة! (لقد توقعت تعادل أو فوز الفريق الأضعف)"
                                  : "Eligible for DOUBLE points! (You predicted the Underdog to Win or Draw)"}
                              </span>
                            ) : (
                              <span
                                className="eligibility-text-standard"
                                style={{ fontWeight: "bold" }}
                              >
                                {isAr
                                  ? "نقاط أساسية عادية فقط. (لقد توقعت فوز الفريق الأقوى المفضل. لتفعيل طاقة مضاعف x2 عليك ترشيح تعادل أو فوز الفريق الأضعف)"
                                  : "Standard Points only. (You predicted the Favorite to win. Select Underdog Draw or Underdog Win to qualify for x2 points multiplier!)"}
                              </span>
                            )}
                          </span>
                        </div>
                            ...
                            {openingChestId === m.match_id ? (
                              ...
                              >
                                <div
                                  style={{
                                    ...
                                    display: "inline-block",
                                    animation:
                                      "chestShake 0.15s ease-in-out infinite",
                                  }}
                                >
                                  🎁
                                </div>
                                ...
                                >
                                  🔓{" "}
                                  {isAr
                                    ? "جاري فتح صندوق الغنائم..."
                                    : "UNLOCKING SURPRISE CHEST..."}
                                </span>
                                <p
                                  style={{
                                    fontSize: "0.7rem",
                                ...
                                >
                                  {isAr
                                    ? "جاري احتساب النقاط والكشف عن الجائزة العشوائية!"
                                    : "Unsealing your gift box & matching results..."}
                                </p>
                              </div>
                            ) : (
                              <div
                              ...
                              >
                                ...
                                >
                                  ✨{" "}
                                  {isAr
                                    ? "لديك صندوق غنائم غير مفتوح!"
                                    : "You have an Unopened Chest!"}
                                </span>
                                <p
                                  style={{
                                    fontSize: "0.72rem",
                                ...
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    try {
                                      const originalGroup =
                                        m.group_stage || "Group Stage";
                                      const newSalt = Math.random()
                                        .toString(36)
                                        .substring(2, 7)
                                        .toUpperCase();
                                      let newGroup = originalGroup;
                                      if (originalGroup.includes("[SALT:")) {
                                        newGroup = originalGroup.replace(
                                          /\[SALT:[^\]]+\]/g,
                                          `[SALT:${newSalt}]`,
                                        );
                                      } else {
                                        newGroup =
                                          `${originalGroup} [SALT:${newSalt}]`.trim();
                                      }
                                      await supabase
                                        .from("matches")
                                    ...
                                    const remainingUnopened = matches.some(
                                      (matchObj) => {
                                        if (matchObj.match_id === m.match_id)
                                          return false;
                                        if (
                                          !matchObj ||
                                          matchObj.match_id ===
                                        ...
                                        const isGroupStage = !isKnockoutStage(
                                          matchObj.group_stage,
                                        );
                                        const isFinished =
                                          matchObj.home_score_final !== null &&
                                          matchObj.away_score_final !== null;
                                        ...
                                        if (
                                          ...
                                          const predVal =
                                            predictions[matchObj.match_id];
                                          const hasExplicitPrediction =
                                            predVal &&
                                            predVal.home !== null &&
                                            predVal.home !== undefined &&
                                            predVal.away !== null &&
                                            ...
                                            if (isExact) {
                                              const chestOpened =
                                                localStorage.getItem(
                                                  `open_chest_${matchObj.match_id}`,
                                                ) !== null ||
                                                (predVal.points_earned !==
                                                  null &&
                                                  predVal.points_earned !==
                                                    undefined);
                                              if (!chestOpened) {
                                ...
                                <>
                                  ...
                                  {hasSurpriseLoot && isExact ? (
                                    ...
                                    >
                                      {basePointsIfExact} + 🎁{" "}
                                      {isAr
                                        ? "جائزة الصندوق المحتملة!"
                                        : "Potential Chest Reward!"}
                                    </span>
                                  ) : (
                                ...
                                <>
                                  ...
                                  Number(pred.away) === m.away_score_final
                                    ? (() => {
                                        ...
                                        if (factorVal < 0.35) {
                                          return isAr
                                            ? "(🎁 غنائم مفاجئة: تم ربح بطاقة مضاعفة 🔋!)"
                                            : "(🎁 Surprise Loot: Earned 1x Double Down Token!)";
                                        } else if (factorVal < 0.7) {
                                          return isAr
                                            ? "(🎁 غنائم مفاجئة: تم ربح بطاقة خبير المستضعفين 🤠!)"
                                            : "(🎁 Surprise Loot: Earned 1x Underdog Specialist Token! 🤠)";
                                        } else {
                                          return isAr
                                            ? "(🎁 غنائم مفاجئة: تم إضافة +٣ نقاط مضمونة!)"
                        ...
                        >
                          ...
                          >
                            ...
                            {expandedOtherPreds[m.match_id]
                              ? t("hideGroupPredictions")
                              : t("seeGroupPredictions")}
                            <span
                              style={{
                                fontSize: "0.7rem",
                                transition: "transform 0.2s",
                                transform: expandedOtherPreds[m.match_id]
                                  ? "rotate(180deg)"
                                  : "rotate(0deg)",
                              }}
                            >
                            ...
                            {expandedOtherPreds[m.match_id] && (
                              ...
                              >
                                ...
                                >
                                  ...
                                  {loadingOtherPreds[m.match_id] ? (
                                  ...
                                  ) : !otherPredictionsMap[m.match_id] ||
                                    otherPredictionsMap[m.match_id].length ===
                                      0 ? (
                                    <div
                                      style={{
                                        textAlign: "center",
                                    ...
                                    >
                                      {otherPredictionsMap[m.match_id].map(
                                        (other) => {
                                          const isSelf =
                                              ...
                                              >
                                                <span
                                                  style={{
                                                    fontSize: "0.813rem",
                                                    fontWeight: "700",
                                                    color: other.has_predicted
                                                      ? "var(--white)"
                                                      : "var(--grey)",
                                                    fontFamily: other.is_hidden
                                                      ? "inherit"
                                                      : "monospace",
                                                  }}
                                                >
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {isAr
                      ? "دليل نقاط خروج المغلوب"
                      : "Knockout Stage Scoring Guide"}
                  </h3>
                </div>
                <button
                  onClick={() => setShowKnockoutModal(false)}
